<?php 
  ...
  $project = isset($_SERVER['BASE_URI']) ? $_SERVER['BASE_URI'] : '';
...
<body>
    ...
    <header class="header">
        <div class="container">
         ...
         <?php if(isset($_SESSION['user'][0]['role']) && $_SESSION['user'][0]['role'] === "admin"): ?>
                <div class="cta-button desktop-only">
    ...
    <?php
       ...
       if($_SERVER['REQUEST_URI'] === "/")
       {
        ...
        //route en GET
        $routes->map("GET",'/deconnexion','User#se_deconnecter');

        $routes->map('GET', '/send-profile', function(){
            echo User::InputType('file','profil','/update/profile/'.$_SESSION['user'][0]['id'],"POST",null,null);
        });
        $routes->map("GET",'/forget-password', function(){
            //echo "vous avez oublie votre mot de passe";
        },'forget-password');
        ...
        // route en POST 
        $routes->map('POST','/route',function(){
        ...
        $routes->map("POST",'/update/profile/[i:id]',function($id){
            
        });

        $routes->map('POST','/update/infos/compte/[i:id]',function($id){
            User::modifier_information_compte($_POST,$_SERVER['REQUEST_METHOD'],(int)$id['id']);
        });

        $routes->map('POST','/UpdatePassword',function(){
            die();
            User::modifier_mot_de_passe($_POST,$_SERVER['REQUEST_METHOD'],(int)$_SESSION['user'][0]['id']);
        });
    ...
    ?>
                <!-- <pre>
                 ...
                 <?= ''//var_dump($_SESSION['user'][0]['id'] ?? false) ?>
                </pre>

                <pre>
                    <?= ''//var_dump(unserialize($_COOKIE['Tokken']) ?? false) ?>
                </pre> -->
    <!-- Footer -->
    <footer class="footer">
        <div class="container">
